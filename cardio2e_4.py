...
## CENARIOS
########
CARDIO2E_FETCH_NAMES_Cenarios = config['cardio2e'].get('fetch_cenarios_name', 'true').lower() == 'true'
CARDIO2E_NCenarios = 40
...
def main():
    try:
        ...
        serial_conn = serial.Serial(DEFAULT_SERIAL_PORT, DEFAULT_BAUDRATE, timeout=2)
...
def on_mqtt_connect(client, userdata, flags, rc):
    ...
    client.subscribe("cardio2e/cenario/set/#")
...
def on_mqtt_message(client, userdata, msg):
    ...
    # Check if the message is for HVAC
        ...
        try:
            ...
            if (float(hvac_states[hvac_id]["cooling_setpoint"])-float(hvac_states[hvac_id]["heating_setpoint"])<2):
                heating_setpoint = float(hvac_states[hvac_id]["cooling_setpoint"]) - 2
            else:
                heating_setpoint = float(hvac_states[hvac_id]["heating_setpoint"])
            cooling_setpoint = float(hvac_states[hvac_id]["cooling_setpoint"])
            fan_state = hvac_states[hvac_id]["fan_state"]
            mode = hvac_states[hvac_id]["mode"]
    ...
    # Verify if a cenario message appears
    elif topic.startswith("cardio2e/cenario/set/"):
        try:
            cenario_id = int(topic.split("/")[-1])
        except ValueError:
            _LOGGER.error("Switch ID invalid on topic: %s", topic)
            return
        if payload == "OFF":
        # Envia comando convertido para o RS-232
            send_rs232_command(userdata["serial_conn"], "M", cenario_id)
                ...
                if received_message:
                    ...
                    for msg in messages:
                        ...
                        if len(message_parts) == 2 and message_parts[0] == "@A":
                            ...
                            elif message_parts[1] == "M":
                                cenario_id = int(message_parts[2])
                                _LOGGER.info("OK for action Cenario: %s", cenario_id)
                            ...
                            if C2E_Typ != "D":
                                get_entity_state(serial_conn, mqtt_client, C2E_Id, C2E_Typ)
                            ...
                            mqtt_client.publish("cardio2e/errors/state", "Sucesso", retain=False)
                        else:
                            _LOGGER.error("Response not processed: %s", message_parts)
                            mqtt_client.publish("cardio2e/errors/state", "Falha.Repita a operação", retain=False)
                            if C2E_Typ != "D":
                                get_entity_state(serial_conn, mqtt_client, C2E_Id, C2E_Typ)
                else:
                    mqtt_client.publish("cardio2e/errors/state", "Falha.Repita a operação", retain=False)
            ...
            while time.time() - start_time < timeout:
                ...
                if received_message.startswith(f"@I N {entity_type}"):
                    ...
                    if entity_type == 'L':
                        ...
                        mqtt_topic = f"cardio2e/cen/name/{entity_id}"
...
def parse_login_response(response, mqtt_client, serial_conn):
    ...
    i = 1
    while i <= CARDIO2E_NCenarios:
    # Obtem nome dos cenarios, se for o caso
        if CARDIO2E_FETCH_NAMES_Cenarios:
            get_name(serial_conn, i, "M", mqtt_client)
        i=i+1
